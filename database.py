...
class Database:
    ...
    def connect() -> Connection:
        ...
        connection: Connection = pymysql.connect(
            user="root", passwd="root", host="localhost", database='COPY_arhtypes_bot_23_aug',
    ...
    def set_newsletter(self, user_id):
        query = f"UPDATE Users SET sending_24_april=NOW() WHERE User_ID={user_id}"
        with self.connect() as connection:
            with connection.cursor() as cursor:
                cursor.execute(query)

    def get_users_for_sending_newsletter(self):
        query = "SELECT User_ID FROM Users WHERE sending_24_april IS NULL AND (Registration_Date + INTERVAL 7 DAY) >= CURDATE();"
        with self.connect() as connection:
            with connection.cursor() as cursor:
                cursor.execute(query)
